...
class Encoder(nn.Module):
    ...
    def forward(self, src):
        ...
        # outputs (tensor containing the features of the input sequence) has shape of (src_len, batch, num_directions * hidden_size)
        # hidden (tensor containing the hidden state for t = seq_len) has shape of (num_layer * num_direction, batch_size, hidden_size)
        outputs, hidden = self.rnn(embedded)

        # hidden has shape of (batch_size, 2 * hidden_size)
        hidden = torch.tanh(
...
class Attention(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        # batch_size x 1 x hid_dim -> batch_size x src_len x hid_dim
        hidden = hidden.unsqueeze(1).repeat(1, src_len, 1)

        # batch_size x src_len x hid_dim
        encoder_outputs = encoder_outputs.permute(1, 0, 2)

        # cat: batch_size, src_len, 3 * hidden_dim
        # attn: batch_size, src_len, hidden_dim
